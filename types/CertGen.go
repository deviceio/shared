...
type CertGen struct {
	...
	// ecdsaCurve ECDSA curve to use to generate a key. Valid values are P224, P256, P384, P521
	EcdsaCurve string
}

func (t *CertGen) publicKey(priv interface{}) interface{} {
	switch k := priv.(type) {
	case *rsa.PrivateKey:
		return &k.PublicKey
...
func (t *CertGen) pemBlockForKey(priv interface{}) *pem.Block {
	switch k := priv.(type) {
	case *rsa.PrivateKey:
